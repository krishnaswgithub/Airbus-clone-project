...
const AllFlights = () => {
  ...
  useEffect(() => {
    if (allSearchState.from && allSearchState.to) {
      const from =
        allSearchState.from.charAt(0).toUpperCase() +
        allSearchState.from.slice(1);
      const to =
        allSearchState.to.charAt(0).toUpperCase() + allSearchState.to.slice(1);
          ...
          allFlights.map((flight) => {
            return (
              <div className="flightContainer" key={uuidv4()}>
                <div className="airlinename width_hundred">
                  {flight.airlineName}
                </div>

                <div className="from width_hundred city_name">
                  {flight.from}
                </div>
